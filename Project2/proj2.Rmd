...
Compute returns using the data for project 1
...
```
Compute means, covariance, variances
```{r}
...
print(paste("D :",D))
```
Part b: Compute lambda1 and lambda2  
```{r}
E = 0.025
...
lambda_1 = l1(E) 
lambda_2 = l2(E)
print(paste("Lambda 1 :",lambda_1))
print(paste("Lambda 2 :", lambda_2))

```
<br>
Part c and d: Find composition with expected return E and plot frontier using parabola method
```{r}
...
```
<br>
Part e: Plot using hyperbola method
```{r}
#Span values of vars 
...
<br>
Part f and g: 
Add 30 stocks, GSPC, equal allocation portfolio, min risk portfolio, portfolio in c, 3 random portfolios
```{r}
